...
namespace Arrays
{
    static class Program
    {
        static void Main1(string[] args)
        {
            ...
            /*
            string message1;               //обычная инициализация
            string message2 = null;        //то же, но с учётом null
            string message3 = System.String.Empty;   //инициализация пустой строки вместо "" или можно просто  string message3 = String.Empty;

            string oldPath = "c:\\Program Files\\tmp.txt";  //инициализация с определенными литералами

            string newPath = @"c:\Program Files\tmp.txt";   //инициализация с вербатим стрингами без доп.преобразований
            ...
            System.String greeting = "Hello World!";      //еще вариант определения не через псевдотип, а через базовый
            string greeting = "Hello World!";             //то же самое
            var greeting1 = "Hello World!";               //==
            const string greeting2 = "Hello World!";      //используем константу
            ...
            char[] letters = { 'A', 'B', 'C' };
            string alphabet = new string(letters);   //создаем строку, передав ей заранее проинициализированный массив символов

            string s1 = "hello";
            string s2 = new String('a', 6); // результатом будет строка "aaaaaa"
            string s3 = new String(new char[] { 'w', 'o', 'r', 'l', 'd' });
            string s4 = new String(new char[] { 'w', 'o', 'r', 'l', 'd' }, 1, 3); // orl     берем элементы только с 1 по 3
        ...
        */
            ...
            //string msg1 = "hello\0";  -- сравниваем посимвольно, поэтому тут строки разные (\0 это null-символ=скрытый, кот.не отображается)
            //string msg2 = "hello";

            Console.WriteLine(msg1 == msg2);                   //true    (строки, как объекты, можно сравнивать по значениям)
            Console.WriteLine(msg1.Equals(msg2));              //используем метод Equals для сравнения
            Console.WriteLine(msg1[0]);                        //тут мы показали, что строка это массив и выведи нам символ 0

            foreach (char c in msg1)                         //перебираем строку в столбик как набор объектов char
            {
                Console.WriteLine(c);
            }
            ...
            //строка вещь неизменяемая, а результатом изменения всегда будет новая строка, примеры:


            string s11 = "A string is more ";
            string s21 = "than the sum of its chars.";

            string s31 = s11 + s21;        //конкатинация=объединение двух сторок с помощью +
            Console.WriteLine(s31);

            s11 += s21;                    //еще вариант без создания третьей строки, а мы как будто в s11 помещаем полученный рез-т
            Console.WriteLine(s11);


            //проверяем через хэшкод, что строка поменялась:
            string s11 = "A string is more ";
            string s21 = "than the sum of its chars.";
            ...
            string s31 = string.Concat(s11, s21, "1111", "!!!!");          //конкатинация=объединение двух сторок с помощью метода Concat
            Console.WriteLine(s31);

            */


            //оформляем текст как многострочный формат без нагромождения спецсимволов:
            ...
            int val = 10;                                                 //текст с интерполяцией $ = текст содержит какие-то значения:
            string text_inter = $"""                                                  
                                 ...
                                 """;
            string text11 = "<element attr=\"content\">" +                //вывод в одну строку, а мы пытались сделать многострочно
                            "<body>" +
                            "</body>" +
                            "</element>";
            ...
            string[] values = new string[] { s5, s6, s7 };   //метод Join используется для объединения элементов коллекции; создает массив значений с разделителем
            ...
            Console.WriteLine($"Compare ==: {comp_str1 == comp_str2}");              //сравнение по значению: true или false
            Console.WriteLine($"Compare Equals: {comp_str1.Equals(comp_str2)}");     //так правильнее (с помощью метода Equals): true или false

            string[] compare_values = { "hello", "world", "about" };        //сравниваем три значения из массива с первоначальным hello
            ...
            Console.WriteLine($"IndexOf 'j' (С учетом регистра): {f_str.IndexOf('j')}"); // поиск символа j по индексу с учетом регистра    
            Console.WriteLine($"IndexOf 'J' (С учетом регистра): {f_str.IndexOf('J')}"); // поиск символа J по индексу с учетом регистра (результат: -1, тк он не найден в исходной строке)
            Console.WriteLine(
            ...
            //IndexOf ведет поиск одного символа
            //IndexOfAny ищет вхождение нескольких символов:


            char[] chars1 = { 'T', 'o', 's' };
            char[] chars2 = { '!', '=', '^' };

            Console.WriteLine($"IndexOfAny 'chars1': {f_str.IndexOfAny(chars1)}");      //перебираем элементы массива chars1
            Console.WriteLine($"IndexOfAny 'chars1': {f_str.IndexOfAny(chars1, 2)}");   //начиная со 2го индекса
            Console.WriteLine($"IndexOfAny 'chars2': {f_str.IndexOfAny(chars2)}");

            Console.WriteLine($"IndexOf 'i': {f_str.IndexOf("just")}");
            Console.WriteLine($"LastIndexOf 'i': {f_str.LastIndexOf('i')}");  //поиск последнего  символа i



            // -=================== StartWith
            var files = new string[]                        //метод StartsWith юзаем ,напр, когда нужно найти какой-либо файл в списке
            {
                "myapp.exe",
                "forest.jpg",
                "main.exe"
            ...
            // -=================== EndsWith
            foreach (var file in files)                      //смотрим, какие из строк заканчиваются определенным образом
            {
            ...
            string text1 = "This is just an ordinary string.";             //наша задача здесь разделить первую и вторую строки на составляющие
            string text2 = "10: 99: :: 55";

            string[] words1 = text1.Split(" ");                                   //разделение по пробелу
            string[] words12 = text2.Split(":");                                  //разделение по :
            string[] words2 = text2.Split(':', StringSplitOptions.TrimEntries);   //TrimEntries удаляет пробелы в начале и в конце строки
            string[] words3 = text2.Split(":", StringSplitOptions.RemoveEmptyEntries);  //удалить пустое значение
            string[] words4 = text2.Split(new char[] { ':', ' ' }, StringSplitOptions.RemoveEmptyEntries); //сплит массива по двум символам

            Console.WriteLine("Ordinary:");
            foreach (var item in words1) Console.Write($"'{item}'\t");     //отдельные элементы в массиве разделены ''
            Console.WriteLine("Ordinary:");
            ...
            string trimmedString = originalString.Trim();                       //нормализация строки - удаляем все лишние символы с начала и в конце
            Console.WriteLine($"Trim: {trimmedString}"); // Вывод: "Hello, world!"

            string originalString1 = "* *Hello, world!* *";
            char[] charsToRemove = { '*', ' ' };                               //удалим все * и лишние пробелы в начале и конце
            ...
            // -=================== TrimStart
            string trimmedStartString = originalString.TrimStart();         //удаляем только начальные пробелы
            Console.WriteLine($"TrimStart: '{trimmedStartString}'"); // Вывод: "Hello, world!   "

            // -=================== TrimEnd                                 //удяляем пробелы только в конце
            ...
            string substring2 = originalSubstringString.Substring(7, 5);                //вырезаем 5 символов, начиная с 7го
            Console.WriteLine($"Substring(7, 5): {substring2}"); // Вывод: "world"

            // Пример 3: Извлечение подстроки с использованием переменных
            int startIndex = 7;
            string world = "world";
            string substring3 = originalSubstringString.Substring(startIndex, world.Length);  //предпочтительный вариант
            ...
            string insertedString = originalInsertString.Insert(7, "beautiful ");  //указываем с какого символа и что нужно вставить, не забываем про пробел
            Console.WriteLine($"Insert (в середине): {insertedString}"); // Вывод: "Hello, beautiful world!"

            // Пример вставки подстроки в начало строки
            string insertedAtStart = originalInsertString.Insert(0, "Greeting: "); //если вставка в начале, то индекс нулевой
            ...
            string removedSubstring1 = originalRemoveString.Remove(7);                //удаление с назначенного индекса и до конца
            ...
            string removedFromEnd = originalRemoveString.Remove(originalRemoveString.Length - 1);  //указываем, сколько символов с конца нужно удалить
            ...
            string replacedString1 = originalReplaceString.Replace("Hello", "Greetings");  //меняет все вхождения символа/строки на новое
            ...
            // -=================== String Builder ===================
            //изменяемый класс, в отличие от string
            ...
            StringBuilder sb1 = new StringBuilder("Hello World!!!");       //конструктор с передачей начального значения
            ...
            // -=================== Append ===================
            StringBuilder sbAppend = new StringBuilder();   //добавили пустую переменную sbAppend
            sbAppend.Append("Hello, ");                     //изменили ее     
            sbAppend.Append("world!");                      //дополнили. как конкатинация
            ...
            // -=================== AppendLine
            //работает как перевод каретки на новую строчку
            ...
            // -=================== Capacity
            //свойство класса StringBuilder, которое предоставляет текущую ёмкость
            ...
            bool isMatch = Regex.IsMatch(input, pattern);       //что (input) чему (pattern) соответствует: содержатся ли в строке символы, соотвествующие нашему паттерну

            if (isMatch)                       //if (isMatch == true) первое значение всегда тру, т.е. это выражение отвечает да
            {
                Console.WriteLine("Строка содержит цифры.");
            }
            else                               //else всегда false
            {
