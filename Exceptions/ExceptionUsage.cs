...
public class ExceptionUsage
{
    ...
    public void Template()
    {
        ...
        catch (Exception ex)                                     //блоки эксепшенов располагать от частного к общему (тут у нас один общий)
        {
    ...
    public void ManyCatch()
    {
        ...
        catch (DivideByZeroException ex)                               //указываем какой тип исключения хотим обработать
        {
            Console.WriteLine($"Возникло исключение {ex.GetType()} {ex.Message}");
        }
        catch (Exception genEx)                                        //общее исключение
        {
    ...
    public void FilterCatch(int x, int y)
    {
        ...
        catch (DivideByZeroException) when (y == 0)            //т.е. если у нас эексепшн такого типа И y равен 0
        {
